...
static void print_version()
{
    ...
    printf( tr("LGeneral %s\nCopyright 2001-%d Michael Speck\nPublished under GNU GPL\n---\n"), VERSION, 2021 );
...
static void add_control(const char *ctlspec)
{
    ...
    if (ctloptnum == ctloptmax) {
	    fprintf(stderr,"Too many command line control options (max: %d)\n",ctloptmax);
	    return;
    }
...
static void eval_cmdline(int argc, char **argv)
{
    ...
    if (optind < argc) {
        ...
        if (load == LOAD_SCEN || load == LOAD_CAMP) {
            ...
            if (ctloptnum > 0) {
                int i;
                for (i = 0; i < setup.player_count && i < ctloptnum; i++) {
                    setup.ctrl[i] = ctlopts[i];
                }
            }
        }
    }
