...
int gui_panel_w = 44;	/* size of fixed GUI panel */;
...
====================================================================
*/
SDL_Surface *gui_create_empty_frame( int w, int h )
{
...
int gui_load( const char *dir )
{
    ...
    /* minimap */
    if ( ( gui->minimap = mmview_create( 132, 132 )) == 0 )
        goto failure;
    ...
    if ( ( gui->unit_buttons = group_create( gui_create_frame( 2*sx+4*dx, 2*sy+2*dy ), 160, load_surf( path2, SDL_SWSURFACE ),
                                             36, 36, 7, ID_SUPPLY, gui->label, sdl.screen, 0, 0 ) ) == 0 )
        goto failure;
    ...
    group_add_button( gui->unit_buttons, ID_RENAME, sx, sy, 0, tr("Rename Unit") ); sx = 3; sy += dy;
    ...
    if ( ( gui->split_menu = group_create( gui_create_frame( 296, 40 ), 200, load_surf( path2, SDL_SWSURFACE ),
    ...
    if ( ( gui->base_menu = group_create( gui_create_empty_frame( 2*sx+dx, 2*sy+8*dy ), 160, load_surf( path2, SDL_SWSURFACE ),
                                          36, 36, 8, ID_MENU, gui->label, sdl.screen, 0, 0 ) ) == 0 )
        goto failure;
    group_add_button( gui->base_menu, ID_AIR_MODE, sx, sy, 0, tr("Switch Air/Ground [t]") ); sy += dy;
    group_add_button( gui->base_menu, ID_STRAT_MAP, sx, sy, 0, tr("Minimap [o]") ); sy += dy;
    group_add_button( gui->base_menu, ID_PURCHASE, sx, sy, 0, tr("Request Reinforcements") ); sy += dy;
    group_add_button( gui->base_menu, ID_DEPLOY, sx, sy, 0, tr("Deploy Reinforcements [d]") ); sy += dy;
    group_add_button( gui->base_menu, ID_UNIT_LIST, sx, sy, 0, tr("Unit List") ); sy += dy;
    group_add_button( gui->base_menu, ID_SCEN_INFO, sx, sy, 0, tr("Scenario Info [i]") ); sy += dy;
    group_add_button( gui->base_menu, ID_CONDITIONS, sx, sy, 0, tr("Victory Conditions") ); sy += dy;
    ...
    if ( ( gui->main_menu = group_create( gui_create_empty_frame( 2*sx+dx, 2*sy+7*dy ), 160, load_surf( path2, SDL_SWSURFACE ),
                                          36, 36, 7, ID_SAVE, gui->label, sdl.screen, 0, 0 ) ) == 0 )
        goto failure;
    group_add_button( gui->main_menu, ID_SAVE, sx, sy, 0, tr("Save Game") ); sy += dy;
    group_add_button( gui->main_menu, ID_LOAD, sx, sy, 0, tr("Load Game") ); sy += dy;
    group_add_button( gui->main_menu, ID_SCEN, sx, sy, 0, tr("Load Scenario") ); sy += dy;
    group_add_button( gui->main_menu, ID_CAMP, sx, sy, 0, tr("Load Campaign") ); sy += dy;
    group_add_button( gui->main_menu, ID_OPTIONS, sx, sy, 0, tr("Options") ); sy += dy;
    group_add_button( gui->main_menu, ID_RESTART, sx, sy, 0, tr("Restart Scenario") ); sy += dy;
    ...
    if ( ( gui->opt_menu = group_create( gui_create_frame( sx*2+dx, 2*sy+7*dy ), 200, load_surf( path2, SDL_SWSURFACE ),
                                          36, 36, 10, ID_C_SUPPLY, gui->label, sdl.screen, 0, 0 ) ) == 0 )
        goto failure;
...
void gui_show_full_info( Unit *unit )
{
    ...
    //y += 10;
    snprintf( str, 128, tr("%s Movement"), mov_types[unit->prop.mov_type].name );
    write_line( contents, gui->font_std, str, x, &y );
    snprintf( str, 128, tr("%s Target"), trgt_types[unit->prop.trgt_type].name );
    write_line( contents, gui->font_std, str, x, &y );
...
void gui_show_conds()
{
    ...
    for ( i = 1; i < vcond_count; i++ ) {
        snprintf( str, 256, "'%s':", vconds[i].message );
        write_line( contents, gui->font_std, str, x, &y );
        for ( j = 0; j < vconds[i].sub_and_count; j++ ) {
            if ( vconds[i].subconds_and[j].player )
                snprintf( str, 256, tr("AND %.2s "), vconds[i].subconds_and[j].player->name );
            else
                snprintf( str, 256, tr("AND -- ") );
            gui_render_subcond( &vconds[i].subconds_and[j], str + 7 );
            write_line( contents, gui->font_std, str, x, &y );
        }
        for ( j = 0; j < vconds[i].sub_or_count; j++ ) {
            if ( vconds[i].subconds_or[j].player )
                snprintf( str, 256, tr("OR %.2s "), vconds[i].subconds_or[j].player->name );
            else
                snprintf( str, 256, tr("OR -- ") );
...
void gui_panel_hide()
{
	...
	/* XXX it's misplaced here but the easy way ... */
	group_hide(gui->unit_buttons,1);
	group_hide(gui->split_menu,1);
	mmview_hide(gui->minimap,1);
}
