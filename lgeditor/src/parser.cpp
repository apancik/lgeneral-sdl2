...
/* disable translation */
#define tr(__TEXT) __TEXT
#define trd(__DOM, __VAL) __VAL
#define DEFSEP 187
#define ITEMSEP 176
...
CommonTreeData *common_tree_data_create(const char *filename) {
    CommonTreeData *ctd = (CommonTreeData*)malloc(sizeof( CommonTreeData));
    ctd->filename = strdup(filename);
    ctd->ref = 0;
    return ctd;
}

void common_tree_data_delete(CommonTreeData *ctd) {
    free(ctd->filename);
}

void common_tree_data_ref(CommonTreeData *ctd) {
    ctd->ref++;
}

int common_tree_data_deref(CommonTreeData *ctd) {
    int del = --ctd->ref == 0;
...
====================================================================
*/
static inline PData *parser_create_pdata( char *name, MyList *values, int lineno, CommonTreeData *ctd )
{
    PData *pd = (PData*)calloc(1, sizeof(PData));
    pd->name = name;
    pd->values = values;
    pd->lineno = lineno;
    pd->ctd = ctd;
...
====================================================================
*/
MyList* parser_split_string( const char *string, const char *symbols )
{
    int pos;
    char *token = 0;
    MyList *list = list_create( 1, 0 );
    while ( string[0] != 0 ) {
        if ( symbols[0] == ' ' )
...
====================================================================
*/
MyList *parser_explode_string( const char *string, char c )
{
    MyList *list = list_create( 1, 0 );
    char *next_slash = 0;
    char small_buf[64];
    while ( *string != 0 && ( next_slash = (char*)strchr( string, c ) ) != 0 ) {
        if ( next_slash != string ) {
	    const unsigned buflen = next_slash - string;
	    /* don't bust the stack. call alloca only on long strings */
	    char *buffer = buflen < sizeof small_buf ? small_buf : (char*)alloca(buflen + 1);
...
void parser_free( PData **pdata )
{
    ...
    if ( (*pdata)->entries ) {
        ...
        while ( ( entry = (PData *)list_next( (*pdata)->entries ) ) )
            parser_free( &entry );
...
====================================================================
*/
PData *parser_insert_new_pdata( PData *parent, const char *name, MyList *values )
{
    PData *pd = parser_create_pdata(strdup(name), values, 0, parent->ctd);

    if (!parent->entries)
        parent->entries = list_create( 0, 0 );
...
int parser_get_pdata  ( PData *pd, const char *name, PData  **result )
{
    ...
    PData *entry = 0;
    char *sub = 0;
    MyList *path = parser_explode_string( name, '/' );
    for ( i = 0, list_reset( path ); i < path->count; i++ ) {
        ...
        while ( ( entry = (PData *)list_iterator_next( &it ) ) )
...
int parser_get_entries( PData *pd, const char *name, MyList   **result )
{
    PData *entry;
    *result = 0;
...
int parser_get_values ( PData *pd, const char *name, MyList   **result )
{
    PData *entry;
    *result = 0;
    if ( !parser_get_pdata( pd, name, &entry ) ) {
...
int parser_get_value  ( PData *pd, const char *name, char   **result, int index )
{
    ...
    *result = (char*)list_get( values, index );
    return 1;
}
